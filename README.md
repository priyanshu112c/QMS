# QMS Interview Presentation — Complete Script

## 15-Minute Presentation for Hiring Managers, BA's, and Developers

---

**Speaker:** A software developer (no pharma background) presenting their understanding of QMS
...
**On Screen:**
> Quality Management System (QMS) in Life Sciences
...
Today, I want to walk you through what I have learned. I will explain the QMS — the Quality Management System — from a developer's perspective. I will cover what it is, why it matters, how the modules work, and how we as developers can build software that truly serves this industry.
...
> Highly regulated — FDA, EMA, MHRA, WHO
> GMP — Good Manufacturing Practice
> Two stages: API Manufacturing → FDF Manufacturing
...
So the entire industry is regulated by government agencies. The FDA in the United States. The EMA in Europe. The MHRA in the UK. The WHO globally. These agencies create rules called GMP — Good Manufacturing Practice.

GMP is not a suggestion. It is the law. Companies that violate GMP get warning letters, fines, and can even be shut down.
...
API stands for Active Pharmaceutical Ingredient. This is the actual chemical that treats the disease. Paracetamol powder is an API. It is made through chemical reactions in large reactors.

FDF stands for Finished Dosage Form. This is what the patient actually takes. The tablet, the capsule, the injection, the syrup. The FDF is made by taking the API powder and mixing it with other ingredients called excipients — things like starch, lactose, magnesium stearate — and then processing it into tablets or capsules.

Let me give you a food analogy. The API is like raw chicken. You cannot eat it directly. The FDF is like chicken curry — the chicken has been cooked with spices and vegetables to make something edible.

The API is made in one factory through chemical processes. The FDF is made in another factory through physical processes like blending, compression, and coating.
...
So now, what is a QMS?

QMS stands for Quality Management System. Here is how I think about it as a developer.

An operating system manages the resources of a computer — files, processes, memory, devices. A QMS manages the resources of quality in a pharmaceutical company — documents, procedures, deviations, CAPAs, audits, training.

It is the system that ensures every medicine is made correctly, tested properly, and is safe for patients.

The QMS has several components. Documents like SOPs — Standard Operating Procedures — that tell people how to do their jobs. Processes like deviation management that handle when something goes wrong. People who are trained and qualified. And records — evidence that everything was done correctly.

The key point is this: QMS is required by law. GMP regulations mandate that every pharmaceutical company must have a QMS. And modern QMS is almost always software-based.

So when we build QMS software, we are building systems that help companies comply with the law and keep patients safe.
...
Let me walk through the QMS modules, starting with In-Process Quality.
...
This is the heart of the QMS. Corrective Action fixes the current problem. Preventive Action stops it from happening again.
...
The software needs to track each action separately, assign owners, set due dates, and — critically — verify that the actions actually worked. This is called effectiveness verification. You monitor for 3 to 6 months. If the problem does not come back, the CAPA is closed. If it does come back, you reopen the investigation.

Regulators love CAPA. When they audit a company, they look at the CAPA system first. If the CAPA system is strong, the rest of the QMS is usually strong too.
...
> QA issues Certificate of Analysis → Batch Released
...
What do we test? The assay — is the API amount correct, usually within 95 to 105 percent of the label claim. The dissolution — does the tablet release the drug properly? Uniformity — is every tablet the same? For injectables — sterility and endotoxins.

If all tests pass, QA issues a Certificate of Analysis, and the batch moves from quarantine to available stock. If any test fails, an OOS — Out of Specification — investigation begins.

From a software perspective, this module integrates with LIMS — the Laboratory Information Management System. Test results can come directly from lab instruments. The system must auto-compare against specifications and flag any failures immediately.
...
The recall process is intense. Assemble a cross-functional team. Notify regulators within 24 hours for Class I. Trace every shipment. Notify every customer. Recover the product. Dispose of it. And investigate root cause through CAPA.
...
**On Screen:**
> AE = Any undesirable medical occurrence in a patient taking your drug
...
The company must record every AE, assess it medically, determine if it is serious and unexpected, and report it to regulators within specific timelines. For a serious unexpected AE in the US, you have 15 calendar days.

The AE database is then data-mined for patterns. If 15 patients develop pancreatitis on your diabetes drug when normally only 2 or 3 would, that is a "safety signal." It triggers investigation and potentially a label change or market withdrawal.
...
Every shipment of raw material is tested when it arrives — Incoming QC. If it passes, it goes to the warehouse. If it fails, it is rejected and returned. And the supplier's performance is tracked — how many lots failed, how often were deliveries on time.
...
> Complaint → Deviation → CAPA → Recall
> All modules connected through a central QMS hub
...
A patient complains that tablets are breaking. That goes into the Complaint module. Investigation confirms a quality issue. That triggers a Deviation. Root cause analysis finds that bottles are underfilled. That leads to a CAPA. The CAPA requires updating the filling specification. That goes through Change Control. The updated SOP requires training. That goes through Training Management.

One issue flows through five or six modules. The software needs to support these connections seamlessly.

Another flow: An In-Process Quality check fails. This triggers a Deviation. The Deviation triggers a CAPA. If the batch is already in the market, it may trigger a Recall.

The key architectural insight for developers is this: every table in your database needs to support cross-module references. A Deviation record needs to link to a Batch, a CAPA, and potentially a Complaint. Your APIs need to support creating these links. Your UI needs to show the connected workflow visually.

---

# SLIDE 14 — Roles in the QMS

**Time:** 11:45 – 12:30

**On Screen:**
> **QA Officer** — Reviews, approves, ensures compliance
> **Production Manager** — Executes manufacturing, reports issues
> **SME (Subject Matter Expert)** — Deep technical knowledge, validates solutions

---

**Script:**

Let me briefly cover the key roles in the QMS.

The QA Officer is the gatekeeper. They review deviations, approve CAPAs, make batch release decisions. In the software, they are the primary approver.
...
The SME — Subject Matter Expert — has deep technical knowledge of the product or process. They help with complex investigations, validate solutions, and define specifications.

Each role sees a different view of the software. The operator sees their task list. The QA Officer sees pending approvals. The Production Manager sees batch status. The QA Manager sees dashboards. Role-based access is not just a UX concern — it is a compliance requirement.
...
**On Screen:**
> API: Paracetamol powder made by chemical synthesis
> FDF: 500mg tablets made by blending → compression → coating → packaging
> IPC checks every 30 min during compression
> Full QC testing before release
...
First, the API. Paracetamol powder is synthesized through a chemical reaction. In-process checks monitor temperature, pH, and conversion rate. The finished API is tested for purity, impurities, and residual solvents. QA releases the API.

Now the FDF. The API powder is blended with excipients — starch as a binder, croscarmellose as a disintegrant, magnesium stearate as a lubricant. IPC checks blend uniformity.
...
After compression, tablets are coated and packaged. IPC checks seal integrity and label accuracy.

The finished batch goes into quarantine. QC tests every specification — assay, dissolution, uniformity. All pass.

QA reviews the complete batch record, including any deviations. Everything checks out. The QA Officer issues the Certificate of Analysis. The batch is released.
...
Fourth, cross-module linking. A Deviation must be able to reference a Batch, link to a CAPA, and connect to a Complaint. My database schema needs foreign keys. My APIs need to support eager loading of related records.

Fifth, role-based access. An operator should not see the QA dashboard. A QA Officer should not see system configuration. I need proper authorization at every layer.
...
And seventh, integration. QMS does not exist in isolation. It connects with ERP for business data, MES for manufacturing data, LIMS for lab data. I need to design robust APIs for these integrations.
...
> ✓ I understand the business — QMS is about patient safety, not just code
...
I now understand that QMS is not just another software module. It is the system that ensures medicines are safe for patients. The code I write directly impacts human health. That is a responsibility I take seriously.
...
| 4 | API vs FDF | 1:30 | 4:00 |
| 5 | QMS Overview | 1:15 | 5:15 |
...
| 3 | GMP is the law — compliance is mandatory |
| 4 | API = the drug molecule, FDF = the form you take |
| 5 | QMS is the operating system for quality |
| 6 | Catch problems early, not at the end |
| 7 | Document everything — even minor issues |
| 8 | CAPA = fix + prevent — the heart of QMS |
...
**Q: Have you worked with 21 CFR Part 11 compliance before?**

A: I have not worked in pharma before, but I have studied it thoroughly. I understand the requirements — audit trails, electronic signatures, system validation. I am confident I can implement these given proper requirements and SME guidance.

**Q: How would you prioritize features for a QMS module?**
...
**Q: What is the most technically challenging part of a QMS system?**
